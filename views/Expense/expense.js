    ...
    if (amount && description && category) {
      try {
        ...
        const response = await axios.post(
          "http://16.171.13.59:3000/expenses/createExpense",
          obj,
          {
  ...
  try {
    ...
    async function getExpenses(page) {
      ...
      const { data } = await axios.get(
        `http://16.171.13.59:3000/getExpenses?page=${page}&limit=${limit}`,
        {
          headers: { Authorization: token },
        }
      );
...
async function ShowOnScreen(res) {
  ...
  delbtn.onclick = async (e) => {
    e.preventDefault();
    try {
      await axios.delete(`http://16.171.13.59:3000/deleteExpense/${res.id}`, {
        headers: { Authorization: token },
      });
      list.removeChild(li);
      Expense.textContent = Number(Expense.textContent) - Number(res.amount);
  ...
  leaderBoardBtn.onclick = async () => {
    try {
      ...
      const leaderBoardData = await axios.get(
        "http://16.171.13.59:3000/premium/showleaderBoard",
        { headers: { Authorization: token } }
      );
...
async function handleDownloadBtn() {
  try {
    const token = localStorage.getItem("token");
    const response = await axios.get("http://16.171.13.59:3000/user/download", {
      headers: { Authorization: token },
    });
...
razBtn.onclick = async function (e) {
  ...
  const response = await axios.get(
    "http://16.171.13.59:3000/purchase/premiummembership",
    { headers: { Authorization: token } }
  );
  ...
  var options = {
    ...
    handler: async function (response) {
      const res = await axios.post(
        "http://16.171.13.59:3000/purchase/updatetransactionstatus",
        {
